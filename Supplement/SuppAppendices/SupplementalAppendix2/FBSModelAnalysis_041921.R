...
ggplot(fold.diff,aes(x=log10(KH.fold),y=log10(KB.fold),fill=log10(HSA.fold)))+theme_bw()+
  geom_raster()+
  scale_fill_gradientn(colors=RColorBrewer::brewer.pal(11,"Spectral"),limits=c(-4.5,3.5),
                       values=c(0,seq(0.4,0.55,length=4),seq(0.58,0.9,length=5),1))
# ggsave("IC50FoldDiff_HSA.png",width=6,height=4)

ggplot(fold.diff,aes(x=log10(KH.fold),y=log10(KB.fold),fill=log10(noHSA.fold)))+theme_bw()+
  geom_raster()+
  scale_fill_gradientn(colors=RColorBrewer::brewer.pal(11,"Spectral"),limits=c(-4.5,3.5),
                       values=c(0,seq(0.4,0.55,length=4),seq(0.58,0.9,length=5),1))
...
ggplot(ex.mlt[ex.mlt$drug.conc>=1e-3&ex.mlt$drug.conc<=1e1,],aes(x=log10(drug.conc),y=frc.bnd,color=condition))+theme_bw()+
  geom_line(size=1.5,alpha=0.8)+
  guides(color=F)
# ggsave("ExampleCurve.png",width=5,height=4)
