...
for (i in 1:length(vars.comb)) {
  ...
  # # Plot single-variable models (change variable in newdata=list(XXXX=x.vals))
  # plot(alpha.df[,vars.i],alpha.df$Actual_Clinical)
  # x.vals = seq(min(alpha.df[,vars.i]),max(alpha.df[,vars.i]),by=0.01)
  # log.pred = predict(mdl,
  #                    newdata=list(free.alpha=x.vals),
  #                    type="response")
  # lines(x.vals,log.pred)
